...
const DeliveryRoute: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (existingScript) {
      console.log(
        "Script do Google Maps já existe, aguardando carregamento..."
      );
    ...
    script.onerror = () => {
      ...
      setError(
        "Erro ao carregar Google Maps. Verifique a API key e o billing."
      );
    };
  ...
  useEffect(() => {
    ...
    const initializeMap = async () => {
      try {
        ...
        directionsService.route(
          ...
          (result, status) => {
            if (status === google.maps.DirectionsStatus.OK && result) {
              ...
              setError(
                "Não foi possível calcular a rota. Verifique se as APIs estão ativadas."
              );
            }
          }
        );
        ...
        setError(
          "Não foi possível carregar o mapa. Verifique se as APIs estão ativadas."
        );
      }
    };
  ...
  useEffect(() => {
    ...
    const computeHeading = (
      ...
      const y =
        Math.sin(((to.lng - from.lng) * Math.PI) / 180) *
    ...
    const updateMarkerVisual = (
      prevPos: google.maps.LatLngLiteral | null,
      newPos: google.maps.LatLngLiteral,
      ...
      try {
        ...
        if (prevPos) {
          ...
          const markerDiv = markerContentRef.current.querySelector(
            "div:first-child"
          ) as HTMLElement | null;
          if (markerDiv) {
        ...
        const badge = markerContentRef.current.querySelector(
          "div:nth-child(2)"
        ) as HTMLElement | null;
        if (badge) {
    ...
    const watchId = navigator.geolocation.watchPosition(
      (pos) => {
        ...
        updateMarkerVisual(
          ...
          newPos,
          currentSteps[currentIdx + 1]?.maneuver
        );
  ...
  const googleMapsUrl = currentPosition
    ? `https://www.google.com/maps/dir/?api=1&origin=${currentPosition.lat},${
        currentPosition.lng
      ...
      >
        ...
        >
          ...
          <div style={{ display: "flex", gap: "8px" }}>
            ...
            >
              <i className="bi bi-check-circle"></i> Confirmar Entrega
            </button>
            <a
              href={googleMapsUrl}
              target="_blank"
      ...
      {!navigationActive ? (
        ...
        >
          ...
          <div style={{ marginTop: "8px", fontSize: "0.85rem", color: "#666" }}>
            A navegação dinâmica será ativada e acompanhará sua localização em
            tempo real
          </div>
        </div>
      ) : (
              ...
              <div style={{ flex: 1 }}>
                <div
                  dangerouslySetInnerHTML={{
                    __html: navigationSteps[currentStepIndex].instruction,
                  }}
                />
                <div
                  style={{
            ...
            <div className="modal-content">
              ...
              <div className="modal-body">
                ...
                <div className="alert alert-info">
                  ...
                  <strong>Nota:</strong> Ao registrar uma anomalia, você
                  retornará ao manifesto de carga.
                </div>
              </div>
